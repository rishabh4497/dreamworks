  ...
  return (
    <div className="rounded-xl border border-separator bg-card p-6">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-[13px] text-muted/80">
            Curated, PIN-locked sub-profiles with restricted access to your library.
          </p>
        </div>
      ...
      <div className="grid gap-4 sm:grid-cols-2">
        {members.length === 0 && (
          <div className="rounded-xl border border-dashed border-separator p-6 text-center text-[12px] text-muted/60">
            No family profiles yet. Add a family member from Settings → Account to share your library.
          </div>
        )}
        {members.map((m) => (
          <div
            key={m.id}
            className="bg-input rounded-xl border border-separator p-4 relative overflow-hidden group cursor-pointer hover:border-indigo-500/50 transition-colors"
          >
